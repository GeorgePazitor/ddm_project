clear all; clc; close all;

% --- Paramètres fixes ---
L = 30; 
S = 1; 
E = 2e5; 
Fd = 10;
tol = 1e-6; 

% Liste des finesses (n = H/h) à tester
n_values = [2, 4, 8, 16, 32, 64, 128]; 

%% ÉTUDE 1 : Cas de l'exercice (H = 10 => N = 3 sous-domaines)
H1 = 10;
results_h_H1 = [];
results_iter1 = [];

fprintf('--- Début Étude 1 BDD (H=%d, N=%d) ---\n', H1, L/H1);
fprintf('%-10s %-10s %-15s\n', 'n', 'h/H', 'Iterations');

for n = n_values
    h = H1 / n;
    
    % Appel de la fonction BDD (Préconditionnée)
    [~, k] = primal_precond_CJ_f(L, S, E, Fd, H1, h, tol);
    
    results_h_H1 = [results_h_H1, h/H1];
    results_iter1 = [results_iter1, k];
    
    fprintf('%-10d %-10.4f %-15d\n', n, h/H1, k);
end

%% ÉTUDE 2 : Cas "Démonstration" (H = 1 => N = 30 sous-domaines)
H2 = 1; 
results_h_H2 = [];
results_iter2 = [];

fprintf('\n--- Début Étude 2 BDD (H=%d, N=%d) ---\n', H2, L/H2);
fprintf('%-10s %-10s %-15s\n', 'n', 'h/H', 'Iterations');

for n = n_values
    h = H2 / n;
    
    % Appel de la fonction BDD (Préconditionnée)
    [~, k] = primal_precond_CJ_f(L, S, E, Fd, H2, h, tol);
    
    results_h_H2 = [results_h_H2, h/H2];
    results_iter2 = [results_iter2, k];
    
    fprintf('%-10d %-10.4f %-15d\n', n, h/H2, k);
end

%% Tracé des graphiques

figure('Name', 'Etude de Scalabilité BDD (h)', 'Color', 'black');

% Subplot 1: Exercise case
subplot(1, 2, 1);
semilogx(results_h_H1, results_iter1, '-bo', 'LineWidth', 2, 'MarkerFaceColor', 'b');
xlabel('h / H (relative mesh size)','Color','w');
ylabel('Number of iterations','Color','w');
title(['Exercise case: N = ', num2str(L/H1), ' (BDD)'],'Color','w');
grid on;
ylim([0 max([results_iter1, results_iter2])+2]); % Same Y-scale for comparison
set(gca, 'XDir', 'reverse');

% Subplot 2: Demonstration case
subplot(1, 2, 2);
semilogx(results_h_H2, results_iter2, '-ro', 'LineWidth', 2, 'MarkerFaceColor', 'r');
xlabel('h / H (relative mesh size)','Color','w');
ylabel('Number of iterations','Color','w');
title(['Demonstration case: N = ', num2str(L/H2), ' (BDD)'],'Color','w');
grid on;
ylim([0 max([results_iter1, results_iter2])+2]);
set(gca, 'XDir', 'reverse');

sgtitle('Question 2.5 : Scalabilité en h (Méthode BDD)');