...
fprintf('--- Question 2.6 : Conditionnement (Version Normalisée) ---\n');
fprintf('%-10s %-10s %-15s\n', 'n', 'h/H', 'cond(M^-1 Sp)');

for n = n_values
    h = H / n;
    
    % --- 1. Construction des matrices Normalisées ---
    % Astuce : On travaille avec k0 = 1 pour la stabilité numérique.
    % Le conditionnement est un rapport, il ne dépend pas de l'échelle E*S/h.
    k0_norm = 1; 
    ...
    Sp_l = cell(1,N);
    for s=1:N 
        % Matrice K locale normalisée
        K_local = k0_norm * (diag(2*ones(n+1,1)) - diag(ones(n,1),1) - diag(ones(n,1),-1));
        K_local(1,1) = k0_norm; K_local(n+1,n+1) = k0_norm;
    ...
    % Inverses locaux (Pseudo-inverses)
    inv_Sp_l = cell(1,N);
    for s=1:N
        % On utilise une tolérance un peu plus large pour pinv pour éviter les fluctuations
        inv_Sp_l{s} = pinv(full(Sp_l{s}), 1e-7);
    end
    
    % Assemblage du préconditionneur
